...
const OpportunitiesPage = () => {
  ...
  const fetchOpportunities = async () => {
    try {
      ...
      toast.error(t('crm.opportunities.errors.load_failed') || 'Failed to load opportunities');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (oppId) => {
    ...
    try {
      ...
      toast.error(t('crm.opportunities.errors.delete_failed') || 'Failed to delete opportunity');
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      toast.error(t('crm.opportunities.errors.update_failed') || 'Failed to update opportunity');
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>{t('crm.opportunities.title') || 'Opportunities'} | IGV CRM</title>
            ...
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-2xl font-bold flex items-center gap-2">
                  ...
                  {t('crm.opportunities.title') || 'Opportunities'}
                </h1>
                <p className="text-sm text-gray-600">
                  {opportunities.length} {t('crm.opportunities.count') || 'opportunities'}
                </p>
              </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-6">
          {loading ? (
            ...
            <div className="bg-white rounded-lg shadow p-12 text-center">
              ...
              <p className="text-gray-600">{t('crm.opportunities.empty') || 'No opportunities yet'}</p>
            </div>
          ) : (
                ...
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">
                      {t('crm.opportunities.columns.name') || 'Name'}
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">
                      {t('crm.opportunities.columns.value') || 'Value'}
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">
                      {t('crm.opportunities.columns.stage') || 'Stage'}
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">
                      {t('crm.opportunities.columns.probability') || 'Probability'}
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">
                      {t('crm.opportunities.columns.created') || 'Created'}
                    </th>
                    ...
                    <tr key={opp.id || opp.opportunity_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${getStageColor(opp.stage)}`}>
                          {t(`crm.opportunities.stages.${opp.stage}`) || opp.stage}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {opp.probability}%
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <h3 className="text-lg font-bold mb-4">
                {t('crm.opportunities.edit') || 'Edit Opportunity'}
              </h3>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="closed_won">{t('crm.opportunities.stages.closed_won') || 'Closed Won'}</option>
                    <option value="closed_lost">{t('crm.opportunities.stages.closed_lost') || 'Closed Lost'}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">
